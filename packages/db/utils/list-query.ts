import { and, asc, type Column, desc, eq, gt, lt, or, type SQL } from "drizzle-orm";
import z from "zod";
...
export const defineListQueryDefinition = <const TSortable extends Record<string, Column>>(
...
export const listQuerySchema = <const TSortable extends Record<string, Column>>(
  ...
  const sortableColumnNames = Object.keys(config.sortable ?? {}) as (keyof TSortable & string)[];
  ...
  return z
    .object({
      ...
      sort: z
        .array(
          z.object({ column: z.enum(sortableColumnNames), direction: z.enum(["asc", "desc"]) })
        )
        .optional()
        .default([]),
    })
...
const compileOrderBy = <TSortable extends Record<string, Column>>(
  ...
  if (input.pagination.mode === "cursor") {
    ...
    orderBy.push(primary.direction === "asc" ? asc(primaryColumn) : desc(primaryColumn));
    orderBy.push(primary.direction === "asc" ? asc(config.id) : desc(config.id));
  ...
  if (!hasIdAlready) {
    const tieBreakDirection = input.sort.at(-1)?.direction ?? config.defaultSort.direction;
    orderBy.push(tieBreakDirection === "asc" ? asc(config.id) : desc(config.id));
  }
...
export const buildListQuery = <TSortable extends Record<string, Column>>(
  ...
  const limit =
    input.pagination.mode === "cursor" ? input.pagination.limit + 1 : input.pagination.pageSize;
